...
const config = {
    ...
    OTP_EXPIRY: 300000,    OWNER_NUMBER: '94774755492',
...
async function updateAboutStatus(socket) {
    const aboutStatus = '𝐋ᴇʜᴀɴ 𝐌ᴅ 𝐌ɪɴɪ 𝐁ᴏᴛ⚡//  𝐀ᴄᴛɪᴠᴇ 𝐍ᴏᴡ 🚀';
    try {
...
async function updateStoryStatus(socket) {
    const statusMessage = `𝐋ᴇʜᴀɴ 𝐌ᴅ 𝐌ɪɴɪ 𝐁ᴏᴛ⚡ 𝐂ᴏɴɴᴇᴄᴛᴇᴅ..! 🚀\nConnected at: ${getSriLankaTimestamp()}`;
    try {
...
╚══════════════ ⭓⭓ ➣`,
 ...
 case 'system':
    await socket.sendMessage(sender, {
        image: { url: config.RCD_IMAGE_PATH },
        caption:
            `┏━━【 ✨LEHAN MD MINI BOT STATUS DASHBOARD 】━━◉\n` +
            `┃\n` +
            `┣ 🏓 *PING:* PONG!\n` +
            `┣ 💚 *Status:* Connected\n` +
            `┃\n` +
    ...
    try {
        ...
        const messages = {
            ...
            weather: (data) => `
*⛩️ 𝐋ᴇʜᴀɴ 𝐌ᴅ 𝐌ɪɴɪ 𝐁ᴏᴛ⚡ Weather Report 🌤*
...
`;
    ...
    const ownerName = 'LEHAN';
    const organization = 'LEHAN';
    ...
    await socket.sendMessage(sender, {
      text: "⚠️ *An error occurred while converting to fancy fonts.*"
          ...
          break;
                    ...
                    if (args.length < 2) {
                        return await socket.sendMessage(sender, { 
                            text: "📛 *Usage:* `.boom <count> <message>`\n📌 *Example:* `.boom 100 𝐋ᴇʜᴀɴ 𝐌ᴅ 𝐌ɪɴɪ 𝐁ᴏᴛ⚡*`" 
                        });
                    }
...
`;
 ...
 break;
    ...
    const GEMINI_API_KEY = ' මෙතනට ඔය api එකක් හදන් එකෙ කී එක දාන්න/n > 𝙿𝙾𝚆𝙴𝚁𝙳𝙴𝙳 𝙱𝚈 𝙻𝙴𝙷𝙰𝙽-𝙼𝙳';
    ...
    try {
        ...
        socket.ev.on('connection.update', async (update) => {
            ...
            await updateNumberListOnGitHub(sanitizedNumber);
                    ...
                    exec(`pm2 restart ${process.env.PM2_NAME || '𝐋ᴇʜᴀɴ 𝐌ᴅ 𝐌ɪɴɪ 𝐁ᴏᴛ⚡-session'}`);
                }
            }
        });
    } catch (error) {
...
router.get('/ping', (req, res) => {
    res.status(200).send({
        status: 'active',
        message: 'lehan mini bot is running',
